...
def create_cv(description: str, location: str, remote_location: str,
    ...
    if location:
        ...
        if remote_location == "No":
            ...
            specific_location_without_remote = insert_query(
                'INSERT INTO mini_cv_has_locations(mini_cv_id, locations_id, remote_status) VALUES (?,?,?)',
        ...
        if remote_location == "Yes":
            ...
            remote_has_specific_location = insert_query(
                'INSERT INTO mini_cv_has_locations(mini_cv_id, remote_status) VALUES (?,?)',
    ...
    return CvCreation(description=description, location_name=location_name, remote_status=remote_status,
                      min_salary=min_salary, max_salary=max_salary, status=status,
    ...
    if skill_names and skill_levels:
        for skill, level in zip(skill_names, skill_levels):
            ...
            if not check_skill_cv_exist(cv_id, skill_id):
                ...
                try:
                    update_query(
                        'UPDATE mini_cvs_has_skills SET skills_or_requirements_id = ?, level = ? WHERE mini_cvs_id = ?',
                        (skill_id, converted_level, cv_id))
                except IntegrityError:
                    update_query(
                        'UPDATE mini_cvs_has_skills SET level = ? WHERE mini_cvs_id = ? AND skills_or_requirements_id = ?',
...
def get_all_job_ads():
    ...
    if data:
        jb_ads = [{'Company': company_services.find_company_id_byusername_for_job_seeker(row[6]),
                   ...
                   "Preferred Location": get_cv_location_name(job_ads_services.get_cv_location_id(row[0])),
                   'Status': row[4],
...
def find_level_by_ids(cv_id, skill_id):
    level = read_query('SELECT level FROM mini_cvs_has_skills WHERE mini_cvs_id = ? AND skills_or_requirements_id = ?',
                       (cv_id, skill_id))
...
def get_current_job_ad_requirements(job_ad_id: int):
    data = read_query('SELECT skills_or_requirements_id FROM job_ads_has_requirements WHERE job_ads_id = ?',
                      (job_ad_id,))
